...
- `run_tatllm_eval.py` — TAT-LLM 7B를 poc_sample_v3.json(1,000문항)에 단일 호출로
  ...
  겹쳐서 폐기하고 train split에서 다시 골랐음). scale이 percent인 산술 문항은
  모델 수식이 이미 x100을 포함했는지 여부가 제각각이라(0.3823 형태 vs 38.23
  형태), 원 저장소 tat_llm_eval.py의 measure_match가 하는 것과 동일하게
  {그대로, x100, /100} 세 후보 중 정답 채점 기준을 통과하는 것을 채택함
  (일반적인 ±1% 오차 허용은 이식하지 않음 — measure_match의 그 부분은 단순
  산술 오차까지 정답 처리해버려서 run_poc.py의 "공식 TAT-QA metric 그대로"
  원칙과 어긋남)
